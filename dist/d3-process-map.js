...
// ================
// authors:
// - nylen/d3-process-map for the original source
// - vanch3d/d3-process-map for the radial constraints and repackaging
//
// A "quick-and-dirty" repackaging of the core layout engine (for D3 v3), ready for distribution and insertion
// Installation:
...
//            <button id="docs-close">
//            <div id="docs"></div>
//        </div>
//    </div>
//
// - use your own CSS or d3-process-map.css (merging the original stylesheets)
// - link both colorbrewer.js and geometry.js
// - Create the config object as per README (and check the default and examples)
// - Use either
//      config.data = { }     for inline data (eg with your own data wrangling)
//      config.jsonUrl = ""   for an AJAX call to the data URL
//   (config.data is checked first, so make sure to keep it null if ajax used)
//
// - simply pass the config to the processmap layout and call start()
//      d3.layout.processmap(config).start();

d3.layout.processmap = function(cfg)
{
    var defaults = {
        "title" : "Default Title",
        "graph" : {
    ...
    function drawGraph() {
        ...
        var display = $graphElt.css('display');
        $graphElt
            .css('display', 'block')
            .css('height', config.graph.height + 'px');
        graph.width  = $graphElt.width()  - graph.margin.left - graph.margin.right;
        graph.height = $graphElt.height() - graph.margin.top  - graph.margin.bottom;
        $graphElt.css('display', display);
        ...
        $graphContainer.on('scroll', function() {
    ...
    function resize(showDocs) {
        ...
        if (typeof showDocs == 'boolean') {
            ...
            $docsContainer[showDocs ? 'show' : 'hide']();
        }

        if (showingDocs) {
            docsHeight = config.docs.desiredDocsHeight;
            $docsContainer.css('height', docsHeight + 'px');
        }

        graphHeight = window.innerHeight - docsHeight;
        $graphContainer.css('height', graphHeight + 'px');

        $docsClose.css({
            top   : graphHeight + config.docs.docsClosePadding + 'px',
            right : window.innerWidth - $docsContainer[0].clientWidth + config.docs.docsClosePadding + 'px'
        });
    }
